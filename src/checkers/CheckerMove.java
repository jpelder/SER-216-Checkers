    ...
    */
    static int isMoveLegal(int[][] board, int srtI, int srtJ, int endI, int endJ, int turn){
    	...
    	if(Math.abs(srtI - endI) == 1){
    		...
    		else if(!(cap_piece == Checkers.redNormal || cap_piece == Checkers.redKing)){ /*same for yellowers*/
    			return illegalMove;
    		}
    ...
    static boolean canCapture(int[][] board, int i, int j){
    	...
    	case Checkers.redNormal:
    		...
    		if(i + 2 < 8 && j + 2 < 8){    
    			if((board[i + 1][j + 1] == Checkers.yellowNormal || board[i + 1][j + 1] == Checkers.yellowKing) && (board[i + 2][j + 2] == Checkers.empty)){
    				return true;
    			}
    		}
    ...
    static boolean canWalk(int[][] board, int i, int j){
    	...
    	case Checkers.redKing:
    	case Checkers.yellowKing:
    		if(isEmpty(board, i + 1, j + 1) || isEmpty(board,i + 1, j - 1) || isEmpty(board, i - 1, j + 1) || isEmpty(board, i - 1, j - 1)){
    			return true;
    		}
    	}
